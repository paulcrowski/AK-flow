...
export async function processUserMessage(params: ProcessInputParams): Promise<ProcessResult> {
  ...
  const agentIdForCache = (memorySpace as MemorySpace | undefined)?.agentId ?? getCurrentAgentId();
  ...
  const globalRecentPromise: Promise<MemoryTrace[]> = wantGlobalBaseline && agentIdForCache
    ? (async () => {
        const cache = (memorySpace as MemorySpace | undefined)?.cold?.cache;
