...
const ALLOWLIST: Array<{ file: string; kind?: Finding['kind']; contains?: string }> = [
  { file: 'core/systems/LimbicSystem.ts', kind: 'let', contains: 'synapticMemory' },
  { file: 'core/systems/TickCommitter.ts', kind: 'let' },
  { file: 'core/systems/TickCommitter.ts', kind: 'newMap', contains: 'lastSpeechByAgent' },
  { file: 'core/systems/DecisionGate.ts', kind: 'newMap', contains: 'gateStateByAgent' },
  { file: 'core/systems/PrismMetrics.ts', kind: 'let', contains: 'dailyPenalty' },
  { file: 'core/systems/FactEchoPipeline.ts', kind: 'let', contains: 'consecutiveFailures' },
  { file: 'core/systems/evaluation/createEvaluationEvent.ts', kind: 'let', contains: 'eventIdCounter' },
  { file: 'core/systems/eventloop/AutonomousVolitionStep.ts', kind: 'let' },
  { file: 'core/systems/ChemistryBridge.ts', kind: 'let', contains: 'activeSubscription' },
  { file: 'core/systems/MemorySpace.ts', kind: 'newMap', contains: 'coldCacheByAgentId' },
  { file: 'core/systems/IntentContract.ts', kind: 'newSet', contains: 'const STYLE' },
  { file: 'core/systems/IntentContract.ts', kind: 'newSet', contains: 'const COMMAND' },
  { file: 'core/systems/IntentContract.ts', kind: 'newSet', contains: 'const URGENCY' },
  { file: 'core/systems/cortex/deepResearch.ts', kind: 'newSet', contains: 'inFlightTopics' },
  { file: 'core/systems/cortex/deepResearch.ts', kind: 'newSet', contains: 'completedTopics' }
];
...
describe('GlobalStateAudit', () => {
  it('should not introduce new module-level mutable state under core/systems', () => {
    const systemsRoot = path.join(process.cwd(), 'core', 'systems');
