...
function App() {
  ...
  useEffect(() => {
    const bootSequence = async () => {
        ...
        // Visual cue that memory is working, but real data is handled in useCognitiveKernel now
        const mems = await MemoryService.recallRecent(1);
        if(mems.length > 0) {
            eventBus.publish({
                id: crypto.randomUUID(),
                timestamp: Date.now(),
  ...
  return (
    ...
    `}>
      ...
      <div className="flex-1 flex flex-col relative z-10">
        ...
        {/* Thought Stream */}
        <div className={`h-12 border-b border-gray-700 flex items-center px-6 text-xs font-mono overflow-hidden whitespace-nowrap transition-colors duration-1000 ${
            systemError ? 'bg-red-900/20 text-red-400' : 
                    ...
                    /* 3. STANDARD MESSAGE OR THOUGHT */
                        ...
                        <div className={`max-w-[80%] p-4 rounded-lg shadow-lg ${
                            msg.role === 'user' 
                                ...
                                : msg.type === 'thought'
                                    ? 'bg-gray-900/60 text-gray-500 italic border border-dashed border-gray-800 text-sm font-mono' 
                                    : 'bg-gray-800 text-gray-200 rounded-bl-none border border-gray-700'
                        }`}>
                            {msg.type === 'thought' && <span className="block text-[10px] text-gray-600 mb-1 uppercase tracking-wider flex items-center gap-1"><Brain size={10}/> Internal Monologue</span>}
                            {msg.text}
                        </div>
                    )}
                </div>
        ...
        <div className={`p-4 border-t transition-colors duration-1000 ${isFatigued ? 'bg-[#151010] border-red-900/20' : 'bg-brain-dark border-gray-700'}`}>
            <div className="relative flex items-center">
                <input 
                    ...
                    className={`w-full bg-gray-800 text-white rounded-full px-6 py-4 pr-12 focus:outline-none focus:ring-2 border placeholder-gray-500 shadow-inner transition-all ${
                        isSleeping 
      ...
      {/* RIGHT: NEURO-MONITOR */}
      <div className="w-[400px] h-full border-l border-gray-800 hidden lg:block">
        <NeuroMonitor limbicState={limbicState} somaState={somaState} resonanceField={resonanceField} />
      </div>
    </div>
  );
}
