...
const TaskCard: React.FC<TaskCardProps> = ({ task, onToggle, onDelete, onEdit, isDragging }) => {
  ...
  return (
    <div 
      className={`group relative p-4 rounded-xl border border-white/5 transition-all duration-200 bg-[#111] hover:bg-[#161616] cursor-pointer ${
    ...
    >
      <div className="flex items-start gap-4">
        ...
        <button 
          onClick={onToggle}
          className={`mt-1 w-5 h-5 rounded border transition-all flex items-center justify-center ${
            task.isCompleted 
...
const TaskColumn: React.FC<TaskColumnProps> = ({ title, type, tasks, accentColor, theme, completedCount, totalCount }) => {
  ...
  return (
    <div className={`flex-1 flex flex-col min-w-[400px] border rounded-2xl bg-black/40 backdrop-blur-sm ${themeColors[theme]}`}>
      {/* Header */}
...
export const TaskBoard: React.FC = () => {
  ...
  const todayTasks = getTasksByType('TODAY');
  const tomorrowTasks = getTasksByType('TOMORROW');
  // Backlog can be hidden or shown in a different view, but for now we focus on the 2 main columns as requested
  
  return (
    <div className="h-full flex flex-col p-6 max-w-[1600px] mx-auto">
      
      {/* Search / Filter Bar Placeholder (static for visual match) */}
      <div className="mb-8 flex items-center justify-between">
         ...
         <div className="flex gap-2">
            <button className="px-3 py-1.5 text-xs font-bold bg-neon-purple text-black rounded shadow-[0_0_10px_rgba(168,85,247,0.4)]">ALL</button>
            <button className="px-3 py-1.5 text-xs font-bold bg-[#111] text-gray-500 hover:text-gray-300 rounded border border-white/5 transition-colors">ACTIVE</button>
            <button className="px-3 py-1.5 text-xs font-bold bg-[#111] text-gray-500 hover:text-gray-300 rounded border border-white/5 transition-colors">DONE</button>
         </div>
      </div>
